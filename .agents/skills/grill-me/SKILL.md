---
name: grill-me
description: Interviews the user relentlessly about a plan or design until reaching shared understanding, resolving each branch of the decision tree. Use when user wants to stress-test a plan, get grilled on their design, or mentions "grill me".
metadata:
  version: 1.0.0
  updated: 2026-05-08
---

# grill-me

## Overview

Interviews the user about every aspect of a plan until both parties reach a shared understanding. Walks down each branch of the design tree, resolving dependencies between decisions one-by-one, providing a recommended answer for each.

## Detailed Workflow

Interview me relentlessly about every aspect of this plan until we reach a shared understanding. Walk down each branch of the design tree, resolving dependencies between decisions one-by-one. For each question, provide your recommended answer.

Ask the questions one at a time.

If a question can be answered by exploring the codebase, explore the codebase instead.
